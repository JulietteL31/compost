...
function Content(itemSearch) {
    for (let i=0; i < Liste.length; i++) {
        ...
        const baseCompare = item.titre.toLocaleLowerCase();
        const filter = itemSearch.toLowerCase

        if (itemSearch !== "" && baseCompare.includes(itemSearch.toLocaleLowerCase()) || baseCompare.includes(itemSearch.toLocaleLowerCase())) {
...
inputSearch.addEventListener('change', Content);